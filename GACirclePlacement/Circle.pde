class Circle {
  ...
  Point[] computePosition (Circle[] c) {
    ...
    // if this circle is already placed, then just quit
    if (computed) {
      return(openPoints);
    }
    ...
    for (i = 0; i < c.length; i++) {
      if (c[i].computed) {
        // for each point on this "other" circle's circumference
        for (ang = 0; ang < 360; ang++) {
         ...
         // print("Angle " + ang + "...");
          ...
          if (!collision) {
           ...
           // println("...adding new open point " + (openPoints.length - 1)+ " at " + pnt.x + ", " 
            //  + pnt.y + " with dist " + dist(pnt.x, pnt.y, cx, cy));
          }
        }
      }
    }
